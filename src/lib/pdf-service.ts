...
export async function processPDF(
  ...
  try {
    ...
    if (contentType.includes("application/zip")) {
      ...
      unlockedArrayBuffer = await zip.files[files[0]].async("arraybuffer");
      wasEncrypted = true;
      ...
      if (!first) return { status: "Failed", error: data.error || "서버 처리 오류" };
  ...
  try {
    ...
    for (const page of pages) {
      ...
      if (options.includeLogo) {
        page.drawText("OROM", { x: 25, y: height - 25, size: 12, font, color: colorBlue });
        page.drawText("edu", { x: 25 + font.widthOfTextAtSize("OROM", 12), y: height - 25, size: 12, font, color: colorRed });
      }
      if (options.includeDate) {
        const dateTextWidth = regularFont.widthOfTextAtSize(today, 10);
        page.drawText(today, { x: width - dateTextWidth - 25, y: height - 25, size: 10, font: regularFont, color: rgb(0.5, 0.5, 0.5) });
      }
    }

    const pdfBytes = await pdfDoc.save();
    const blob = new Blob([pdfBytes], { type: "application/pdf" });
    return { status: wasEncrypted ? "Unlocked" : "Already Unlocked", blob };
